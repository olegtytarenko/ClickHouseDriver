    ...
    public function sendAsyncFilesWithQuery(Server $server, string $query, array $files, int $concurrency = 5): array
    {
        $requests = function ($files) use ($server, $query) {
            foreach ($files as $file) {
                ...
                $request = new Request(
                    'POST', $this->buildRequestUri($server, [
                        'query' => $query, 
                        'max_block_size' => 7516192768,
                        'distributed_aggregation_memory_efficient' => $server->getOptions()->getDistributedAggregationMemoryEfficient(),
                        'max_memory_usage' => $server->getOptions()->getMaxMemoryUsage(),
                        'max_query_size' => $server->getOptions()->getMaxQuerySize(),
                        'max_bytes_before_external_group_by' => $server->getOptions()->getMaxBytesBeforeExternalGroupBy()
                    ]), $headers, $this->getFileHandle($file)
                );
